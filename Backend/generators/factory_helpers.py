...
def construct_dicebear_api_url(style, content_type="png", seed=None):
    ...
    if style == "notionists-neutral":
        ...
        def variant(prefix, count):
            return [f"{prefix}{str(i).zfill(2)}" for i in range(1, count + 1)]

        params = {
            "backgroundColor": random.choice(
                ["ffffff", "b6e3f4", "ffd5dc", "d1d4f9", "fff5b7", "c0aede"]
            ),
    ...
    url = f"{dice_bear_api}{style}/{content_type}?{query}"
    return url
