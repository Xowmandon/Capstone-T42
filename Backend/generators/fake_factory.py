...
class FactoryManager:
    ...
    def gen_fake_swipe(self, swiper, swipee, result):
        ...
        new_swipe = models.swipe.Swipe(
            swiper_id=swiper, swipee_id=swipee, swipe_result=result
        )
    ...
    def create_swipable_user(self, user_id: str, swipe_result=None):
        ...
        print(
            f"  Location: {existing_user.city}, {existing_user.state}, {existing_user.country_code}"
        )
        ...
        print(
            f"  Location             : {new_user.city}, {new_user.state}, {new_user.country_code}"
        )
    ...
    def create_match(self, user1_id: str, user2_id: str):
        ...
        existing_match = models.match.Match.query.filter(
            (models.match.Match.matcher == user1_id)
            & (models.match.Match.matchee == user2_id)
        ).first()
    ...
    def create_photos(self, is_main_photo, style, N_tot=20):
        """Create main photos for users."""
        photo_factory = PhotoFactory(image_format="png")
        ...
        if is_main_photo:
            ...
            for user_id in self.user_ids:
                ...
                db_urls = [
                    _photo.url
                    for _photo in models.photo.UserPhoto.query.filter_by(
                        user_id=user_id
                    ).all()
                ]
                ...
                print("URLS: " + str([photo.url for photo in photos]))
                print(f"LENGTH: {len(photos)}")

            print("Final Page: " + str(self.page))
            return self.page
...
class CLIGenerator:
    ...
    def display_help(self):
        ...
        print()
        print(
            "  stream --users                - Create a Creation Stream of users (Profile, Main & Gallery Photos)"
        )
        print(
            "  create --users                - Create n swipable users with PENDING state"
        )
    ...
    def handle_create_swipable_users(self):
        ...
        try:
            ...
            result = (
                input(
                    "Enter the swipe result (y for PENDING, r for Rejected, a for Accepted): "
                )
                .strip()
                .lower()
            )
            ...
            for _ in range(count):
                self.factory_manager.create_swipable_user(
                    existing_user_id, swipe_result=swipe_result
                )
        except ValueError:
    ...
    def handle_create_matches(self, user1_id):
        ...
        try:
            ...
            if len(factory_manager.user_ids) < 2:
                print(
                    "Not enough users to create matches. Please create more swipable users first."
                )
                return
            match_ids = []
            for i in range(count):
                user2_id = self.factory_manager.user_ids[
                    (i + 1) % len(factory_manager.user_ids)
                ]
    ...
    def handle_populate_main_photos(self, file_paths, N_tot: int):
        ...
        responses = self.factory_manager.populate_main_photos(
            ...
            file_names=[f"Fake_main_photo_{i}.png" for i in range(N_tot)],
    ...
    def handle_create_main_photos(self):
        ...
        file_paths = self.factory_manager.create_photos(
            style=selected_style, is_main_photo=True
        )
        if len(file_paths) == 0:
    ...
    def handle_create_gallery_photos(self):
        ...
        total = int(
            input(
                "Enter the number of gallery photos to create per User Generated: "
            ).strip()
        )
        ...
        page = self.factory_manager.create_photos(
            is_main_photo=False, style=query, N_tot=total
        )
    ...
    def handle_show_user_matches(self, user_id):
        ...
        matches = models.match.Match.query.filter(
            (models.match.Match.matcher_id == user_id)
            | (models.match.Match.matchee_id == user_id)
        ).all()
        ...
        if matches:
            ...
            for match in matches:
                ...
                other_user_id = (
                    match.matchee_id
                    if match.matcher_id == user_id
                    else match.matcher_id
                )
                other_user_name = (
                    models.user.User.query.filter_by(id=other_user_id).first().name
                )
                ...
                print(
                    f"Last Messager ID: {lst_msg.messager_id if lst_msg else 'No messages yet'}"
                )
            ...
            if command == "exit":
                ...
                for _ in range(count):
                    factory_manager.create_swipable_user(
                        main_user_id, swipe_result=choice(["PENDING", None, None, None])
                    )
                # Print the user IDs
                ...
                cli_generator.handle_populate_main_photos(
                    file_paths, N_tot=len(factory_manager.user_ids)
                )
