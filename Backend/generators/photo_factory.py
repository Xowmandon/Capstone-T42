...
class PhotoPaths:
    def __init__(self):
        ...
        self.USER_PHOTOS_MAIN_PATH = os.path.join(
            self.USER_PHOTOS_BASE_PATH, "MainPhotos/"
        )
        self.USER_PHOTOS_GALLERY_PATH = os.path.join(
            self.USER_PHOTOS_BASE_PATH, "GalleryPhotos/"
        )
        self.FAKE_PHOTOS_MAIN_PATH = os.path.join(
            self.USER_PHOTOS_BASE_PATH, "FakeMainPhotos/"
        )
        self.FAKE_PHOTOS_GALLERY_PATH = os.path.join(
            self.USER_PHOTOS_BASE_PATH, "FakeGalleryPhotos/"
        )
...
class PhotoFactory:
    ...
    def locally_upload_user_photo(
        self, data, file_path, filename, is_main_photo, is_fake_photo=False
    ):
        """Construct the upload path for User Photo Upload file."""

        save_path = self.path_manager.get_path(
            filename=filename, is_main_photo=is_main_photo, is_fake_photo=is_fake_photo
        )
    ...
    def gen_photos(
        ...
        for _ in range(count):
            ...
            photo_url = construct_dicebear_api_url(
                style=style, content_type="png", seed=seed
            )
    ...
    def get_pexels_photos(self, query="Gaming, Anime", page=1, count=3):
        """Fetch a single photo from Pexels matching the query."""
        print("get_pexels_photos -Page: ", page)
        # Pexels API URL and headers
        base_url = "https://api.pexels.com/v1/search"
        query = "Game 8-Bit Avatars, Safe for Work"
        headers = {"Authorization": self.pexels_key}

        # Encode the query string
        # This is important to ensure the query is properly formatted for the URL
        ...
        try:
            ...
            response.raise_for_status()
            response.json().get("next_page", None)
    ...
    def gen_gallery_photos(
        self, page, query="8-bit landscape", api="pexels", per_page=3
    ) -> list:
