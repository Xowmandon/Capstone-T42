...
class GenFake():
    ...
    def get_fake_users(self):
        return models.User.query.filter_by(fake=True).all()
    ...
    def is_matched(self,user1, user2):
        ...
        user1_matches = models.Match.query.filter_by(matcher=user1.email).all()
        user2_matches = models.Match.query.filter_by(matchee=user2.email).all()
    ...
    def is_swiped(self,user1, user2):
        ...
        user1_swipes = models.Swipe.query.filter_by(swiper=user1.email).all()
        user2_swipes = models.Swipe.query.filter_by(swipee=user2.email).all()
    ...
    def gen_fake_match(self):
        while True:
            ...
            if not self.is_matched(pair_users[0], pair_users[1]):
                ...
                match = models.Match(matcher=matcher, matchee=matchee, match_date=self.fake.date_time())
                return match
    ...
    def gen_fake_swipe(self,swiper):
        ...
        swipe = models.Swipe(swiper=swiper, swipee=swipee, swipe_result=random.choice([1, 0]), swipe_date=self.fake.date_time())
...
if __name__ == '__main__':
    ...
    test_fake_users = [UserGenerator.gen_fake_user() for i in range(100)]
    ...
    with open(csv_f, 'a') as f:
        for user in test_fake_users:
            try:
                ...
                print(f"Error: {e}")
                continue
            #f.write(f"{user.to_csv()}\n")  # Write each user on a separate line
            f.write( str(user_schema.dump(user)))
            f.write("\n")

    pair_users = tuple(random.sample(test_fake_users, 2))
    match = models.Match(matcher=pair_users[0], matchee=pair_users[1], match_date=UserGenerator.fake.date_time())
    
    match_schema = schemas.MatchSchema(only=('matchee', 'match_date'))
    print(match_schema.dump(match))
    #match_schema(only=('matchee', 'match_date')).dump(match)
   # match_schema.dump_only = ('matcher', 'matchee', 'match_date')
