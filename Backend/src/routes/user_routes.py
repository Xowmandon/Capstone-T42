...
from Backend.src.models.user import UserProfileSchema
from Backend.src.routes.route_helpers import (
    validate_required_params,  # Import the Models and Schemas
)
...
def init_preferences():
    ...
    dating_pref_exists = (
        db.session.query(models.datingPreference.DatingPreference)
    ...
    try:
        ...
        return jsonify({"error": "Database Error - {e1}"}), 500

    except Exception as e:
        print(f"Invalid Data Provided - {e}")
        return jsonify({"error": "Invalid Data Provided - {e}"}), 400
...
def init_profile():
    ...
    try:
        ...
        print(f"User Profile Data: {json.dumps(profile_data, indent=4)}")
...
def upload_profile_picture():
    ...
    description = request.form.get(
        "description", type=str, default="Default Description"
    )
...
def get_profile_pictures():
    ...
    try:
        ...
        main_photo = (
            db.session.query(models.photo.UserPhoto)
            .filter_by(user_id=user_id, is_main_photo=True)
            .order_by(models.photo.UserPhoto.upload_date.desc())
            .first()
        )
        ...
        jsonify(
            {
                ...
                "user_photos": (
                    [photo.url for photo in additional_photos]
                    if additional_photos
                    else []
                ),
