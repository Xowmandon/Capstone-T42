...
class UserClusteringService:
    ...
    def gen_user_dset(self):
        ...
        messages = models.message.Message.query.filter(
            messager_id=models.user.User.id
        ).all()
        messages_dict = {}
    ...
    def label_users(self):
        ...
        for idx, (self.df["combined"], label) in enumerate(
            zip(self.df["combined"], self.labels)
        ):
