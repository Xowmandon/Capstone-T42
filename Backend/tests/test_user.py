...
from Backend.tests.utils_for_tests import (
    pytest_assertion_failure,
    pytest_assertion_success,
...
def test_email_signup_route(client):  # Test the Email Signup Route
    ...
    assert response.status_code == 201, pytest_assertion_failure(
        f"Unexpected status code: {response.status_code} - {response.data}"
    )  # Check that the status code is 201 (Created)
    assert response.json["token"] is not None, pytest_assertion_failure(
        "'Token' Not Found in Response JSON"
    ...
    for msg in [
        ...
        "Signup Success Found in Response",
    ]:
...
def test_get_user_route(client):
    ...
    with Backend.app.app.app_context():
        ...
        assert response.status_code == 200, pytest_assertion_failure(
            f"Unexpected Response:{response.status_code}{response.data}"
        )
        assert "id" in response.json, pytest_assertion_failure(
            "Response JSON does not contain 'id'"
        )
        assert (
            db.session.get(User, response.json["id"]) is not None
...
def test_home_route(client):  # Test the Home Route
    ...
    assert response.status_code == 200, pytest_assertion_failure(
        f"- {response.status_code} - Server is Not Responding, Might be Offline!"
    )
    assert (
        response.json["message"] == "Welcome to the UnHinged API!"
...
def test_get_swipe_pool(client):
    ...
    # Send a GET request to the get user endpoint with X-Authorization Header as JWT
    response = client.get(
        "/users/swipe_pool", headers={"X-Authorization": f"Bearer {test_jwt}"}
    )
...
def test_get_matches(client):
    ...
    # Send a GET request to the get user endpoint with X-Authorization Header as JWT
    response = client.get(
        "/users/matches", headers={"X-Authorization": f"Bearer {test_jwt}"}
    )
